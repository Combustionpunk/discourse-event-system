...
class CreateDesEventPayouts < ActiveRecord::Migration[7.0]
  def change
    unless table_exists?(:des_event_payouts)
    ...
    unless index_exists?(:des_event_payouts, :event_id, unique: true)
      add_index :des_event_payouts, :event_id, unique: true
    end

    unless index_exists?(:des_event_payouts, :organisation_id)
      add_index :des_event_payouts, :organisation_id
    end

    unless index_exists?(:des_event_payouts, :status)
